...
async function buildMaterialLookup(): Promise<{
  ...
  for (const m of materials) {
    const match: MaterialMatch = { material_id: m.id, material_name: m.material_name };

    if (m.normalized_name) {
      byName.set(m.normalized_name.toLowerCase(), match);
    }
    if (m.material_name) {
      byName.set(m.material_name.toLowerCase(), match);
    }
    if (m.cas_rn) {
...
export async function ingestPlastChem(dbPath?: string): Promise<void> {
  ...
  for (const entry of entries) {
    ...
    for (const target of targets) {
      ...
      if (error) {
        // Likely null CAS conflict, try with chemical name as pseudo-CAS
        if (!entry.cas_rn) {
